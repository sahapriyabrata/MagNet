...
parser.add_argument('--seed', default=525, help='Seed for random numbers')
...
parser.add_argument('--musigma', default=None, help='Path to trainset musigma')
...
with torch.no_grad():
    while t < seq_len//2 + 200:
        ...
        error = np.mean(np.square(truth[:,:2] - pred[:,:2]))
        print('Frame: {}, Error: {}'.format(t,error))
        obsr_error = np.mean(np.square(truth[:,:2] - obsr[:,:2]))
